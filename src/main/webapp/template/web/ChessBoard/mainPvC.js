...
function setupBoardSquares() {
  for (let i = 0; i < boardSquares.length; i++) {
    // Các sự kiện drag and drop hiện có
    boardSquares[i].addEventListener("dragover", allowDrop);
    boardSquares[i].addEventListener("drop", drop);
    
    // Thêm sự kiện khi di chuột vào ô
    boardSquares[i].addEventListener("click", handleMouseEnter);
    // boardSquares[i].addEventListener("mouseleave", handleMouseLeave);
...
function handleMouseEnter(event) {
  ...
  // const squareId = event.target.id; // ID của ô hiện tại
  const squareDiv = event.currentTarget;
  const squareId = squareDiv.id;
  const pieceElement = squareDiv.querySelector('.piece'); // Tìm quân cờ trong ô hiện tại

  if (pieceElement) {
    const pieceColor = pieceElement.getAttribute("color");
    const pieceType = pieceElement.classList[1];
    const pieceObject = { pieceColor, pieceType, pieceId: pieceElement.id };
    
    // Lấy danh sách các ô hợp lệ
    const legalMoves = getPossibleMoves(squareId, pieceObject, boardSquaresArray);
    
    // Tô màu các ô hợp lệ
    legalMoves.forEach(move => {
      const legalSquare = document.getElementById(move);
      if (legalSquare) {
        legalSquare.classList.add('highlight'); // Thêm class để tô màu
      }
    });
  }
}
...
function drop(ev,MoveToClient=0) {
  ...
  if (pieceType == "king") {
    ...
    if (legalSquares.length == 0){
        console.log("Quan Co Khong Co Nuoc Di");
    }else {
      console.log("Cac Nuoc di la: "+legalSquares);
    }
  if (
...
function isMoveValidAgainstCheck(legalSquares,startingSquareId,pieceColor,pieceType){
  ...
  console.log(boardSquaresArrayCopy);
